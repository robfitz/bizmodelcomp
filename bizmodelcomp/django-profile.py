#!/usr/bin/env python
# -*- coding:utf-8 -*-
# $Id: django-profile.py 294 2007-11-19 21:57:32Z dogada $
...
"""
TODO:
- take in account Site for absolute urls
- add follow_redirects options
- parse <link> also
- integrate in web-site doctests
"""
...
def profile_size(url):
    ...
    if response.status_code == 200:
        img = len(re.findall(r'<img.*?>', response.content))
        link = len(re.findall(r'<link.*?>', response.content))
    if options.verbosity:
        print ", size: %sKb, status code: %s " %\
              (size, response.status_code)
    return {'Size, b': size, 'Status': response.status_code,
            '<img>': img, '<link>': link}
...
def get_urls_from_content(urls):
    ...
    for url in urls:
        response = _internal_request(url)
        if response.status_code == 200:
            for path in iter_page_urls(response.content, url):
                hrefs.add(path)
    debug('hrefs', hrefs)
    return hrefs
